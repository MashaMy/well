...
class sct_cleanup_site_orphaned_relationships extends sct_cleanup {
    ...
    public function run($args = array(), $stats = array()) {
        ...
        $sql = sprintf("DELETE tr FROM %s tr INNER JOIN %s tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.taxonomy not in ('link_category', 'ngg_tag') AND tr.object_id NOT IN (SELECT ID FROM %s);", $wpdb->term_relationships, $wpdb->term_taxonomy, $wpdb->posts);
    ...
    public function check($args = array()) {
        ...
        $sql = sprintf("SELECT count(*) as records, SUM(LENGTH(tt.taxonomy)) * 3 as size FROM %s tr INNER JOIN %s tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.taxonomy not in ('link_category', 'ngg_tag') AND tr.object_id NOT IN (SELECT ID FROM %s);", $wpdb->term_relationships, $wpdb->term_taxonomy, $wpdb->posts);
        $data = $wpdb->get_row($sql);
        sct_log('sql_check', 'site', $sql, get_class($this), 'log');
...
class sct_cleanup_site_db_overhead extends sct_cleanup {
    ...
    public function help($args = array()) {
        $help = '<ul>';
        $help .= '<li>'.__("Overhead cleanup is available only for database tables not using InnoDB engine.", "smart-cleanup-tools").'</li>';
        $help .= '</ul>';
