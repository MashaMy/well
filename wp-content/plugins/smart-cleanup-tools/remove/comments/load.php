    ...
    public function get_notice($args = array()) {
        ...
        switch ($args['remove']) {
            case 'remove_pingbacks':
                ...
                break;
            case 'remove_unapproved':
                $items[] = __("Tool will remove all unapproved (pending) comments.", "smart-cleanup-tools");
                break;
    ...
    public function get_title($args = array()) {
        ...
        switch ($args['remove']) {
            case 'remove_pingbacks':
                ...
                break;
            case 'remove_unapproved':
                $render .= __("Remove unapproved comments", "smart-cleanup-tools");
                break;
            case 'clean_useragent':
    ...
    public function check($args = array()) {
        ...
        switch ($args['remove']) {
            case 'remove_pingbacks':
                ...
                break;
            case 'remove_unapproved':
                $sql = sprintf("SELECT COUNT(*) as records, SUM(LENGTH(comment_content) * 2 + LENGTH(comment_agent)) as size FROM %s c LEFT JOIN %s m ON c.comment_ID = m.comment_id WHERE comment_approved = '0'", $wpdb->comments, $wpdb->commentmeta);
                break;
            case 'clean_useragent':
    ...
    public function remove($args = array()) {
        ...
        switch ($args['remove']) {
            case 'remove_pingbacks':
                ...
                break;
            case 'remove_unapproved':
                $sql = sprintf("DELETE c, m FROM %s c LEFT JOIN %s m ON c.comment_ID = m.comment_id WHERE comment_approved = '0'", $wpdb->comments, $wpdb->commentmeta);
                break;
            case 'clean_useragent':
