...
class Mega_News_Ticker extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'News Ticker', 'elementor-mega-posts-display' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'News Ticker Type', 'elementor-mega-posts-display' ),
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Item Views', 'elementor-mega-posts-display' ),
	...
	if($vcmp_newsticker_excerpt_number == '') { $vcmp_newsticker_excerpt_number = '50'; }
		...
		if($vcmp_newsticker_type == 'newsticker1') {
			...
			if($vcmp_custom_style == 'on') {
				...
				$return .= ".posts_newsticker_type1.admpselector-$instance li {
		...
		$return .= '<div class="adclear"></div><div class="admp-newsticker-'.$instance.' admegaposts posts_newsticker_type1 admpselector-'.$instance.'">'; // OPEN MAIN DIV
		$return .= '<i class="fa fa-angle-up fa-2x" id="newsticker-prev-'.$instance.'"></i>';
		$return .= '<ul id="admp-newsticker-type1-'.$instance.'">';
		$count = 0;
		...
		$return .= '<a href="'.get_the_permalink().'" title="'.get_the_title().'">'.vcmegaposts_thumbs().'</a>';
		$return .= '</div>';	
        
		}
        
        $return .= '<div class="mega-info">
        ...
        $return .= '<div class="mega-date">';
		$return .= vcmegaposts_post_info($vcmp_newsticker_date,$vcmp_newsticker_comments,$vcmp_newsticker_author,$vcmp_newsticker_category,$vcmp_newsticker_views,$vcmp_query_source,$vcmp_query_posts_type,$vcmp_newsticker_date_format);        
        $return .= '</div>';
		...
		if($vcmp_custom_style == 'on') {
			...
			$return .= ".posts_newsticker_type2.admpselector-$instance li {
		...
		$return .= '<div class="adclear"></div><div class="admp-newsticker-'.$instance.' admegaposts posts_newsticker_type2 admpselector-'.$instance.'">'; // OPEN MAIN DIV
		$return .= '<ul id="admp-newsticker-'.$instance.'">';
		$count = 0;
		$loop = new \WP_Query($query);
		if($loop) { 
		while ( $loop->have_posts() ) : $loop->the_post();		
		...
		$link = get_permalink(); 		
		?>
        <?php $return .= '<li data-infos="'.vcmegaposts_excerpt($vcmp_newsticker_type2_excerpt_number).'">'; 
        
        $return .= '<i class="icon-play3"></i><span class="hour">'.get_the_date($vcmp_newsticker_date_format).'</span><a href="'.$link.'">'.get_the_title().'</a>';
		...
		if($count2 == '0') {
				$return .=	'<div id="newsticker-infos-container'.$instance.'" class="newsticker-info">
									...
									<div id="newsticker-infos-'.$instance.'" >
										...
										<div class="ad_three_fourth ad_last">
											<div class="infos-text">'.vcmegaposts_excerpt($vcmp_newsticker_type2_excerpt_number).'</div>
										</div>
									</div>
